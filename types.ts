...
/**
 * Cách tính điểm cho câu Đúng/Sai:
 * - 'equal'   : chia đều — mỗi ý đúng = điểm/câu ÷ 4
 * - 'stepped' : thang bậc BGD — 1ý=10%, 2ý=25%, 3ý=50%, 4ý=100%
 */
...
export interface SectionPointsConfig {
  ...
  // Cách tính điểm Đúng/Sai (chỉ dùng khi questionType = 'true_false')
  // Mặc định: 'stepped' (thang bậc BGD)
...
/**
 * Cấu hình chi tiết cho phòng thi
 */
export interface RoomSettings {
  allowLateJoin: boolean;              // Cho phép vào muộn
  showResultAfterSubmit: boolean;      // Hiển thị kết quả sau khi nộp
  shuffleQuestions: boolean;           // Xáo trộn câu hỏi
  maxAttempts: number;                 // Số lần làm bài tối đa
  allowAnonymous: boolean;             // Cho phép thi không cần đăng nhập
  
  // ✅ MỚI: Cấu hình xem đáp án và lời giải
  showCorrectAnswers: boolean;         // Cho phép xem đáp án đúng sau khi nộp
  showExplanations: boolean;           // Cho phép xem lời giải chi tiết
  
  // ✅ MỚI (TÙY CHỌN): Cấu hình nâng cao
  showAnswersAfterClose?: boolean;     // Chỉ cho xem đáp án sau khi đóng phòng
  allowReview?: boolean;               // Cho phép xem lại bài làm
}
...
export interface Room {
  ...
  // ✅ CẢI TIẾN: Nhóm các cấu hình vào settings
  settings: RoomSettings;
  
  // ✅ GIỮ LẠI (backward compatibility) - sẽ dần chuyển sang settings
